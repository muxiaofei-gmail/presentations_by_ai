...
import SlideOutro1 from './slide-outro-1.vue'
import SlideOutro from './slide-outro.vue'
import XidijiShowcase3000Card from '../../shared/product-card/cards/XidijiShowcase3000Card.vue'
...
const presentationContext = createPresentationContext({
  ...
  contextKey: 'xidiji-showcase-3000',
  persist: true
})

providePresentationContext(presentationContext)

// 总页数：3个开头页 + 产品数量 + 2个结尾页
const INTRO_SLIDES = 3
const OUTRO_SLIDES = 2
const totalSlides = INTRO_SLIDES + products.length + OUTRO_SLIDES
const currentIndex = ref(0)

// 当前页面类型
const currentPageType = computed(() => {
  if (currentIndex.value < INTRO_SLIDES) return 'intro'
  if (currentIndex.value >= totalSlides - OUTRO_SLIDES) return 'outro'
  return 'product'
})
...
const currentOutroIndex = computed(() => {
  if (currentPageType.value === 'outro') {
    return currentIndex.value - (totalSlides - OUTRO_SLIDES)
  }
  return -1
})
  ...
  <div class="presentation-page">
    ...
    <div class="content-wrapper">
      ...
      >
        ...
        <!-- 结尾页 -->
        <SlideOutro1 v-else-if="currentPageType === 'outro' && currentOutroIndex === 0" />
        <SlideOutro v-else-if="currentPageType === 'outro' && currentOutroIndex === 1" />
      </div>
    </div>
